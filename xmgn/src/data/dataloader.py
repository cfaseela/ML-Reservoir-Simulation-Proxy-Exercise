...
def compute_global_statistics(graph_files, stats_file=None):
    ...
    Patched: streaming Welford's algorithm replaces the original which concatenated
    every per-node feature from every graph into one huge tensor before computing
    mean/std (OOMs at ~9 GB for 60 cases of Norne data; would scale to ~70 GB for
    500 cases). The streaming approach holds only running (count, mean, M2) per
    category — O(num_features) memory, regardless of dataset size. Mathematically
    identical result (Welford is exact, not approximate).
    ...
    """
    logger.info(
        f"Computing global statistics (streaming Welford) across {len(graph_files)} graphs..."
    )

    def update(count, mean, M2, batch):
        """Welford streaming update for a batch [N, F]."""
        n = batch.shape[0]
        if n == 0:
        ...
        if batch_mean.shape != mean.shape:
            return count, mean, M2  # skip mismatched feature-dim
        new_count = count + n
        delta = batch_mean - mean
        new_mean = mean + delta * n / new_count
        new_M2 = M2 + batch_M2 + (delta * delta) * count * n / new_count
        return new_count, new_mean, new_M2

    nc, nm, nM2 = 0, None, None
    ec, em, eM2 = 0, None, None
    tc, tm, tM2 = 0, None, None

    for i, fp in enumerate(graph_files, 1):
        try:
            g = torch.load(fp, weights_only=False)
            if hasattr(g, "x") and g.x is not None:
                nc, nm, nM2 = update(nc, nm, nM2, g.x)
            if hasattr(g, "edge_attr") and g.edge_attr is not None:
                ec, em, eM2 = update(ec, em, eM2, g.edge_attr)
            if hasattr(g, "y") and g.y is not None:
                tc, tm, tM2 = update(tc, tm, tM2, g.y)
            if i % 200 == 0:
                logger.info(
                    f"  Processed {i}/{len(graph_files)} graphs (running nodes={nc:,})"
                )
        except Exception as e:
            logger.warning(f"Failed to load graph {fp}: {e}")
            continue

    def _finalize(count, mean, M2):
        if mean is None or count < 2:
            return torch.tensor([]), torch.tensor([])
        return mean, torch.sqrt(M2 / (count - 1))  # unbiased

    node_mean, node_std = _finalize(nc, nm, nM2)
    edge_mean, edge_std = _finalize(ec, em, eM2)
    target_mean, target_std = _finalize(tc, tm, tM2)

    logger.info(f"  Node features:   {node_mean.numel()} dims, {nc:,} total nodes")
    logger.info(f"  Edge features:   {edge_mean.numel()} dims, {ec:,} total edges")
    logger.info(f"  Target features: {target_mean.numel()} dims, {tc:,} total nodes")
    ...
    # ---- ORIGINAL (concatenating) implementation kept below for reference, unused ----
    _UNREACHABLE = True
    if _UNREACHABLE:
        return  # never executed; the streaming version above always returns first
    # Collect all node, edge, and target features (legacy)
    all_node_features = []
    all_edge_features = []
    all_target_features = []
