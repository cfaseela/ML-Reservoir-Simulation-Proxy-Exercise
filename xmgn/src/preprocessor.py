...
class ReservoirPreprocessor:
    ...
    def create_partitions_from_graphs(self, graph_file_list=None):
        ...
        for i, graph_file in tqdm(
            ...
            # PATCHED #4 (idempotent partition skip):
            # Resumable skip — if this partition file already exists on disk
            # AND we have this case's partition_assignment in memory (so the
            # per-case JSON will still get written), skip the expensive
            # torch.load + METIS + halo + torch.save. This lets us resume
            # after a mid-run crash without redoing completed work.
            _basename = os.path.basename(graph_file)
            _case_name = self._extract_case_name_from_filename(_basename)
            _partition_file = os.path.join(
                self.partitions_dir, f"partitions_{_basename}"
            )
            if (
                _case_name in partition_assignments_by_case
                and os.path.exists(_partition_file)
                and os.path.getsize(_partition_file) > 0
            ):
